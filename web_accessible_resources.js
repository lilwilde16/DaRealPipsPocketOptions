  ...
  function _install(ws) {
    ...
    ws.addEventListener('message', function(evt) {
      ...
      if (_data instanceof ArrayBuffer && eng.action) {
        ...
        try {
          var _s = JSON.parse(_data.slice(4));
          if (!Array.isArray(_s)) return;
          if      ('updateHistoryNew'     === _s[0]) { eng.action = 'updateHistory'; }
          else if ('updateStream'         === _s[0]) { eng.action = 'updateStream'; }
          else if ('updateAssets'         === _s[0]) { eng.action = 'updateAssets'; }
          else if ('successupdateBalance' === _s[0]) { eng.action = 'updateBalance'; }
          else if ('updateOpenedDeals'    === _s[0]) { eng.action = 'updateOpenedDeals'; }
          else if ('successopenOrder'     === _s[0]) { eng.action = 'successopenOrder'; }
          else if ('successcloseOrder'    === _s[0]) { eng.action = 'successcloseOrder'; }
          else if ('upsignals'            === _s[0] || 'updateSignalForecast' === _s[0] ||
                   'signals/load'         === _s[0] || 'signals/update'       === _s[0]) {
            eng.action = 'signals';
          }
        } catch (_) {}
      }
    });
    console.log('[MPB] retroactive engine listener installed on pre-existing socket url=' + (ws.url || '?'));
  }
  var _pool = window.__mpbWsPool || [];
  for (var _i = 0; _i < _pool.length; _i++) _install(_pool[_i]);
  console.log('[MPB] retroactive patch: checked ' + _pool.length + ' pool socket(s)');
})();
...
window.addEventListener('message', function(ev) {
  ...
  // When the resolved WebSocket lacks oldSend (not engine-wrapped — e.g. a pre-existing
  // socket captured before the constructor override ran), ws.send() bypasses the engine
  // interceptor and the deal's pair/direction/amount would never be applied.  Fall back to
  // PATH B: pop the deal and send it directly with the correct values via sendOrder().
  if (!ws.oldSend && window.__mpbAutoTrader && typeof window.__mpbAutoTrader.sendOrder === 'function') {
    var poppedDeal = eng.userInfo.futureDeals.pop();
    if (poppedDeal) {
      console.log('[MPB] auto-trade (PATH B fallback) — pair:', poppedDeal.pair, 'dir:', poppedDeal.dur);
      var res = window.__mpbAutoTrader.sendOrder(poppedDeal.pair, poppedDeal.dur, poppedDeal.sum);
      if (res.ok) {
        eng.userInfo.openedDials++;
      } else {
        console.warn('[MPB] auto-trade PATH B failed:', res.reason, '— re-queuing deal');
        eng.userInfo.futureDeals.push(poppedDeal);
      }
    }
    return;
  }
  ...
  function mountSystemCheck(){
    ...
    copyDbgBtn.addEventListener('click', function(){
      var snippet = [
        ...
        '      console.log("  take_profit.sum:", st.take_profit ? st.take_profit.sum : "n/a");',
        ...
        '    console.log("TradeWS:", tw ? ("readyState=" + tw.readyState + " hasOldSend=" + !!tw.oldSend) : "MISSING");',
        '    console.log("WS pool:", p.length, "sockets");',
        '    for (var i = 0; i < p.length; i++) {',
        '      console.log("  pool[" + i + "] readyState=" + p[i].readyState + " hasOldSend=" + !!p[i].oldSend + " engListener=" + !!p[i].__mpbEngineListener + " url=" + (p[i].url || "?"));',
        '    }',
