...
// === RSI 30s Dual Strategy — inject option into native strategy select ===
(function(){
  function injectRsiDual30sOption(){
    var sel = document.querySelector('#sub-menu-robot-modal select, #sub-menu-robot-modal .po-strategy-select');
    if(!sel || sel.querySelector('[value="rsiDual30s"]')) return;
    var opt = document.createElement('option');
    opt.value = 'rsiDual30s';
    opt.textContent = 'RSI 30s Dual (6+14)';
    sel.appendChild(opt);
    // Ensure selecting this option updates the bot strategy setting and selected_pairs
    sel.addEventListener('change', function(){
      if(this.value === 'rsiDual30s'){
        window.postMessage({belobot:true, act:'setState', settings:{strategy:'rsiDual30s'}}, window.location.href);
      }
    });
  }
  injectRsiDual30sOption();
  var mo = new MutationObserver(injectRsiDual30sOption);
  mo.observe(document.documentElement, {childList:true, subtree:true});
})();
