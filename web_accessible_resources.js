/* ================= MONEY PRINTER BOT - CORE TRADING ENGINE ================= 
 * This file contains the core trading logic, strategies, and WebSocket interception.
 * 
 * Purpose:
 * - Intercepts WebSocket connections to monitor market data and trades
 * - Implements multiple trading strategies (signals, CCI, candles, pin bar)
 * - Manages automated deal execution with Martingale support
 * - Handles balance tracking and deal history
 * - Communicates with the UI overlay via window.postMessage
 * 
 * Trading Strategies Included:
 * 1. Signals - Based on platform signal strength across multiple timeframes
 * 2. CCI (Commodity Channel Index) - Momentum-based indicator strategy
 * 3. Candles - Candlestick pattern recognition
 * 4. Pin Bar - Price action pattern detection
 * 
 * Key Components:
 * - settings: Trading configuration (strategy, limits, take-profit, etc.)
 * - userInfo: Account balance, deals, and user data
 * - strategies: Implementation of trading algorithms
 * - WebSocket interceptor: Captures and modifies platform communication
 * 
 * Note: This runs in the page context (not content script context) to access
 * the page's WebSocket connections and trade execution functions.
 * ================================================================== */
(function(){try{
